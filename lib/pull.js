...
const execAsyncWithTimeout = (command, options = {}, timeout = 60000) => {
  console.log({command, options, timeout})
  return Promise.race([
    execAsync(command, options),
    new Promise((_, reject) => {
      setTimeout(() => reject(new Error(`Command timed out: ${command} after ${timeout / 1000}s`)), timeout)
    }),
  ])
}
  ...
  return queue.add(async () => {
    ...
    try {
      ...
      if (existsSync(projectPath)) {
        ...
        await execAsyncWithTimeout(`git clone ${repoUrl} ${projectPath}`, {}, 1800000) // 30 min
        console.info(`Installing dependencies for ${projectPath}`)
        await execAsyncWithTimeout(`npm ci`, { cwd: projectPath })
      }
    } catch (error) {
